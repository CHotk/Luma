...
class SmokingRepository {
  SmokingRepository(this._store);

  static const _key = 'smoking.entries.v1';

  final KeyValueStore _store;

  Future<List<SmokingEntry>> _loadAllRaw() async {
    final raw = await _store.read(_key);
    if (raw == null) return <SmokingEntry>[];
    return (jsonDecode(raw) as List)
        .cast<Map<String, dynamic>>()
        .map(SmokingEntry.fromJson)
        .toList();
  }

  Future<void> _write(List<SmokingEntry> all) =>
      _store.write(_key, jsonEncode([for (final e in all) e.toJson()]));

  /// 給 UI 用：已刪除的濾掉，新的在前面。
  Future<List<SmokingEntry>> loadAll() async {
  ...
  /// 給同步用：連刪除標記都要看得到。
  Future<List<SmokingEntry>> allForUpload() => _loadAllRaw();

  Future<SmokingEntry> add({String? reason, DateTime? at}) async {
  ...
  Future<int> mergeFromCloud(List<SmokingEntry> incoming) async {
