...
import '../../domain/smoking_stats.dart';
...
/// 抽菸記錄（2026-09-24 使用者要求：記錄多久一次，督促自己不要太頻繁；
/// 看盤／抽菸／喝酒各自獨立一份，不共用程式）。版面取自設計稿
/// `design-history/看盤頻率記錄設計/`：03 的「距離上次」計時器＋冷靜按鈕、
/// 01 的最近紀錄清單、02 的月曆日期格子；05 的統計放在右上角統計按鈕進去的子頁。
class SmokingPage extends ConsumerStatefulWidget {
  const SmokingPage({super.key});

  @override
  ConsumerState<SmokingPage> createState() => _SmokingPageState();
}

class _SmokingPageState extends ConsumerState<SmokingPage> {
  List<SmokingEntry> _entries = const [];
  bool _loaded = false;
  Timer? _tick;
  ...
  Future<void> _load() async {
    final all = await ref.read(smokingRepositoryProvider).loadAll();
    if (mounted) {
      setState(() {
        _entries = all;
        _loaded = true;
          ...
          child: Column(
            ...
            children: [
              ...
              Wrap(
                ...
                runSpacing: 8,
                children: [
                  for (final r in const ['壓力', '無聊', '飯後', '社交', '習慣', '其他'])
                    ActionChip(
                      label: Text(r),
  ...
  Future<void> _confirmDelete(SmokingEntry e) async {
    ...
    if (ok != true) return;
    await ref.read(smokingRepositoryProvider).delete(e.id);
    await _load();
  }
            ...
            child: Column(
              ...
              children: [
                ...
                AppTopBar(
                  ...
                  actions: [
                    IconButton(
                      onPressed: () => context.push('/smoking-log/stats'),
  ...
  Widget _buildTimerCard() {
    ...
    return GlassCard(
      ...
      child: Column(
        children: [
          Text('距離上次抽菸', style: AppText.note),
          const SizedBox(height: Gap.sm),
          Text(
